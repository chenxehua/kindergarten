...
test.describe('课程管理功能测试', () => {
  ...
  test('TC-COURSE-001: 课程列表页面应该正常显示', async ({ page }) => {
    ...
    page.on('response', response => {
      if (response.url().includes('/api/course') && response.request().method() === 'GET') {
    ...
    console.log('Course API requests:', apiRequests.length);
  ...
  test('TC-COURSE-002: 检查课程列表元素', async ({ page }) => {
    const apiRequests = [];
    page.on('response', response => {
      if (response.url().includes('/api/course')) {
        apiRequests.push({ url: response.url(), status: response.status() });
      }
    });
    ...
    console.log('Course elements - API requests:', apiRequests.length);
  });
  ...
  test('TC-COURSE-011: 课程API数据验证', async ({ page }) => {
    let apiData = null;
    let apiStatus = 0;
